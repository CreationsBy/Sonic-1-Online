...
A private-room, maximum-four-player web experience for the original **Sonic the Hedgehog** on Sega Genesis. Each browser runs its own emulator, so players can move through the game independently. The server relays only position/stage metadata and short-lived reconnect checkpoints; it never receives the ROM.
...
4. Wait for the host to press **Start race**.
...
This is asynchronous “race ghost” multiplayer, not traditional shared-input Genesis netplay:
...
- Those small values go through WebSocket. A peer on the same act is drawn as a colored Sonic ghost at `peer world position - local camera position`. Off-screen peers get an edge marker. Players in another act remain visible in the race-status panel.
...
- `public/js/app.js` controls ROM validation, lobby UI, synchronized launch, race HUD, and notifications.
...
The tests cover the four-player limit, host-only start, reconnect seat/checkpoint retention, ROM/name validation, stage-clear transitions, byte-accurate RAM extraction, and Apple/mobile detection including modern iPadOS user agents.
