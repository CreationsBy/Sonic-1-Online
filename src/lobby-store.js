...
export class LobbyStore {
  ...
  join({ code, name, token, romHash, socket }) {
    ...
    if (lobby.status !== "lobby") {
      throw new LobbyError("ALREADY_STARTED", "This co-op game has already started.");
    }
  ...
  beginStart(code, playerId, delayMs = 2500) {
    ...
    if (lobby.status !== "lobby") {
      throw new LobbyError("BAD_STATE", "The game is already starting or playing.");
    }
  ...
  updateTelemetry(code, playerId, input) {
    ...
    if (telemetry.mode === 0x18 && player.currentStage?.key === "6:0" && !player.completedGame) {
      player.completedGame = true;
      player.spectatingId = null;
      for (const spectator of lobby.players) {
        if (spectator.spectatingId === player.id) spectator.spectatingId = null;
      }
      finished = { player: publicPlayer(player, lobby.hostId) };
    }

    return { accepted: true, telemetry, clear, finished };
  }

  setSpectating(code, playerId, targetId) {
    const lobby = this.requireLobby(code);
    const spectator = this.requirePlayer(lobby, playerId);
    if (!spectator.completedGame) {
      throw new LobbyError("NOT_FINISHED", "Finish the game before entering spectator mode.");
    }

    if (targetId === null || targetId === undefined || targetId === "") {
      spectator.spectatingId = null;
      return { lobby, spectator, target: null };
    }

    const target = this.requirePlayer(lobby, String(targetId));
    if (target.id === spectator.id || target.completedGame || !target.connected) {
      throw new LobbyError("BAD_SPECTATOR_TARGET", "Choose a connected teammate who has not finished yet.");
    }
    spectator.spectatingId = target.id;
    lobby.touchedAt = this.now();
    return { lobby, spectator, target };
  }
